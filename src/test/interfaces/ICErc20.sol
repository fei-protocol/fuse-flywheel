...
interface CErc20 {
  ...
  function _setImplementationSafe ( address implementation_, bool allowResign, bytes memory becomeImplementationData ) external;
  function _setInterestRateModel ( address newInterestRateModel ) external returns ( uint256 );
  function _setNameAndSymbol ( string memory _name, string memory _symbol ) external;
  ...
  function initialize ( address underlying_, address comptroller_, address interestRateModel_, string memory name_, string memory symbol_, uint256 reserveFactorMantissa_, uint256 adminFeeMantissa_ ) external;

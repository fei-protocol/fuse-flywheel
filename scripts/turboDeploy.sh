...
POOL_8_STRATEGY=$(forge create FuseERC4626 --constructor-args $POOL_8_FEI_C_TOKEN $POOL_8_NAME $POOL_8_SYMBOL --rpc-url "http://127.0.0.1:8545/" --private-key $PRIVATE_KEY | grep 'Deployed to:' | awk '{print $NF}')

# 2. Deploy Pool 18 Fei strategy
POOL_18_STRAGEGY=$(forge create FuseERC4626 --constructor-args $POOL_18_FEI_C_TOKEN $POOL_18_NAME $POOL_18_SYMBOL --rpc-url "http://127.0.0.1:8545/" --private-key $PRIVATE_KEY | grep 'Deployed to:' | awk '{print $NF}')
